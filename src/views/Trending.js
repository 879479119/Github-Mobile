...
export default class Trending extends Component {
  ...
  changeLang(e) {
    // I don't like the design of selection, so it's replaced with menu
    if (e.target.getAttribute('data-lang') === undefined) return
  ...
  render = () => {
    ...
    const p = location.search.match(/span=(\w*)/)
    ...
    const shownLanguages = ['C', 'HTML', 'Java', 'Javascript', 'Python', 'Ruby', 'TypeScript']

    const LinkWrap = function ({ lang, text }) {
          ...
          <section className="t-left-part">
            ...
            >
              ...
              <SubMenu title={<span><Icon type="filter" />Time</span>}>
                <Menu.Item key="daily"><Link to={`/trending${developer ? '/developers' : ''}/${language}?span=daily`}>by
                  Daily
                                       </Link>
                </Menu.Item>
                <Menu.Item key="weekly"><Link to={`/trending${developer ? '/developers' : ''}/${language}?span=weekly`}>by
                  Weekly
                                        </Link>
                </Menu.Item>
                <Menu.Item key="monthly"><Link
                  to={`/trending${developer ? '/developers' : ''}/${language}?span=monthly`}
                >by
                  Monthly
                                         </Link>
                </Menu.Item>
            ...
            <ul onClick={::this.changeLang}>
              ...
              <li key="unknown"><LinkWrap span={span} lang="unknown" text="Unknown languages" /></li>
              {
                shownLanguages.map(item => (
                  <li key={item.toLowerCase()}><LinkWrap
                    span={span} lang={item.toLowerCase().replace(' ', '+')}
                    text={item}
                  />
                  </li>
                ))
