...
function dataFormat(commits) {
  ...
  const totalDay = 52 * 7 + weekToday + 1
  const firstDay = new Date(today.getTime() - totalDay * 24 * 60 * 60 * 1000)
  // all day should be concerned

  const tableData = []
  for (let i = 0; i < totalDay; i++) { tableData.push(0) }

  commits.forEach((item) => {
    const d = new Date(item.time)
    if (d >= firstDay) {
      const index = Math.floor((d.getTime() - firstDay.getTime()) / 24 / 60 / 60 / 1e3)
      tableData[index]++
    }
  })
...
export default function () {
  const data = dataFormat(DATA.data)
        ...
        <tbody>
          {
            data.map((item) => (
              <tr>
                {item.map((t, j) => <td className={`level-${t >= 4 ? 4 : t}`} />)}
              </tr>
            ))
          }
        </tbody>
