		...
		<ul className={cls("pr", className)} {...props}>
			{
				...
				list.map((item, index) => {
					return (
						<Item item={item} key={index} base={base}/>
					)
				})
			}
		</ul>
...
class Item extends Component{
	...
	render = () => {
		const {item, index, base} = this.props
		return (
			<li className="issue-item">
				<section className="i-s-main">
					...
					<p>#{item.number} opened {fromNow(item.created_at)} by <Link to={`/user/${item.user.login}/profile`}>{item.user.login}</Link></p>
				</section>
