...
export default class PunchChart extends Component{
	render(){
		let {data, className, margin= [160, 20], parent: {width, height}, ...props} = this.props
		...
		let max = 0
		for(let i = 0;i < arr.length;i ++) arr[i][2] > max ? max = arr[i][2] : 0
		...
		return (
			<svg width={width} height={height} {...props} className={cls("chart-punch", className)}>
				{
					weeks.map((row, index)=>{
						return <Row key={index} margin={margin} index={index} row={row} label={weekday[index]} width={innerWidth} max={max} />
					})
				}
				<g transform={`translate(0,490)`}>
					{
						timeline.map((item, i)=>(
							<text key={i} x={margin[0] + i * (width - margin[0] - margin[1] - 20) / 24 - 10} y={24}>{item}</text>
						))
					}
				</g>
			</svg>
...
function Row({row, label, width, index, margin, max}) {
	let lineHeight = 70
	return (
		<g transform={`translate(0,${index*lineHeight})`}>
			...
			{
				row.map((t, i)=>{
					let posX = margin[0] + i * (width - margin[0] - margin[1]) / 24
					let r = 16 / max * t[2]
					return (
						<g key={i}>
							<line x1={posX} x2={posX} y1={lineHeight - 8 - (i % 2 === 0 ? 3 : 0)}
							      y2={lineHeight} stroke="#ddd" shapeRendering="crispEdges"/>
